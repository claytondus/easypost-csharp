...
namespace EasyPost {
    public class Batch : Resource {
        ...
        public async Task AddShipments(IEnumerable<Shipment> shipments) {
            await AddShipments(shipments.Select(shipment => shipment.id).ToList());
        }
        ...
        public async Task RemoveShipments(IEnumerable<Shipment> shipments) {
            await RemoveShipments(shipments.Select(shipment => shipment.id).ToList());
        }
        ...
        public async Task GenerateLabel(string fileFormat, string orderBy = null) {
