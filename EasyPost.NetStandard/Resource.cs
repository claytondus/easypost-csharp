    ...
    public class Resource : IResource {
        ...
        public static T LoadFromDictionary<T>(Dictionary<string, object> attributes) where T : Resource {
            ...
            foreach (PropertyInfo property in type.GetTypeInfo().DeclaredProperties) {
                ...
                if (property.PropertyType.GetTypeInfo().ImplementedInterfaces.Contains(typeof(IResource))) {
                ...
                } else if (typeof(IEnumerable<IResource>).GetTypeInfo().IsAssignableFrom(property.PropertyType.GetTypeInfo())) {
                    property.SetValue(resource, Activator.CreateInstance(property.PropertyType), null);

                    Type genericType = property.PropertyType.GetTypeInfo().GenericTypeParameters[0];
